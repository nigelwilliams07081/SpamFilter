...
void MPI_Send_string(std::string s, int dest, int tag) {
	int len = s.size();
...
void MPI_Send_bool(bool b, int dest, int tag) {
	char val = (char)b;
	MPI::COMM_WORLD.Send(&val, 1, MPI::UNSIGNED_CHAR, dest, tag);
}

std::string MPI_Recv_string(int source, int tag, MPI::Status &status) {
	MPI::Status s;	
...
bool MPI_Recv_bool(int source, int tag, MPI::Status &status) {
	char val;
	MPI::COMM_WORLD.Recv(&val, 1, MPI::UNSIGNED_CHAR, source, tag, status);
	return (bool)val;
}

bool MPI_Recv_bool(int source, int tag) {
	char val;
